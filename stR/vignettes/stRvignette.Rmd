...
```{r, results='hide', message=FALSE, echo=FALSE, warning=FALSE, cache=TRUE}
require(forecast)
require(seasonal)
```
```{r, fig.show='hold', fig.width = 7, fig.height = 4.5, cache=TRUE}
...
You can notice that it works much longer that any other above mentioned methods. It happens partially because the <code>AutoSTR</code> function tries to estimate the best parameters of the corresponding STR model. If the parameters are known, the execution is much faster:

```{r, fig.show='hold', fig.width = 7, fig.height = 3, cache=TRUE}
# fit.STR = STR(co2, lambdas = list(c(2,0,0),(1,2,2)))
# plot(fit.STR)
```
